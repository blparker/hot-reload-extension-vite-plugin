...
var chalkLogger = {
  green: (message) => {
    console.log(source_default.bgGreen(`[${PLUGIN_NAME}] ${message}`));
  },
  red: (message) => {
    console.log(source_default.blue.bgRed(`[${PLUGIN_NAME}] ${message}`));
  }
};

// src/utils/websocket.ts
function initAndListenConnection(cb) {
  const wss = new import_ws.WebSocketServer({ port: HOT_RELOAD_EXTENSION_VITE_PORT });
  ...
  return {
    ...
    async transform(code, id) {
      ...
      if (sidepanelPath && id.includes(sidepanelPath)) {
        const buffer = import_fs.default.readFileSync((0, import_path.resolve)(__dirname, "scripts/sidepanel-reload.js"));
        return {
          code: code + buffer.toString()
        };
      }
...
var main_default = hotReloadExtension;
